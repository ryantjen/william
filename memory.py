...
def store_memory(text: str, metadata=None, project: str | None = None):
    """
    Store memory with optional project tag in metadata.
    """
    final_metadata = dict(metadata or {})
    if project:
        final_metadata["project"] = project

    # include project in id to reduce collisions
    mem_id = str(hash(text + "|" + (project or "")))
...
def store_memories(texts: list[str], metadatas: list[dict], project: str | None = None):
    """
    Batch store many chunks in ONE call (much faster, fewer embedding calls).
    """
    if not texts:
        return
    ...
    for i, (t, md) in enumerate(zip(texts, metadatas)):
        ...
        ids.append(str(hash(t + "|" + (project or "") + "|" + str(i))))
